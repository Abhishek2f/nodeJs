...
app.put("/jokes/:id", (req, res)=>{
  ...
  jokes.forEach((jokeObj)=>{
    if(jokeObj.id === id){
      jokeObj.jokeText = req.body.text;
      jokeObj.jokeType = req.body.type;
      res.json(jokeObj);
      flag = true;
    }
  });
  if(!flag){
    res.send({"message": "id not found to be updated"});
  }
})
...
app.patch("/jokes/:id", (req, res)=>{
  ...
  //construct the patch update
  if(jokeIndex === -1){
    res.json({"message": "joke Id is not found try correct Id"});
  }else{
...
app.delete("/all", (req, res)=>{
  ...
  if(key === masterKey){
    ...
    res
    .status(200)
    .json({message : "Joke list has been deleted"});
  }else{
    res
    .status(404)
